...
> Hint: Add the type `ITurboPost` to `@dpc-turbo/types` package and import from there when needed.

```ts
interface ITurboPost {
  id: number;
  title: string;
  body: string;
  tags: string[];
...
```ts
interface AddTurboPostRequestData {
  title: string;
  body: string;
  tags?: string[];
}
...
> Hint: Add the following list to `apps/dpc-next/src/util/dummy-posts.ts` and import from there when needed.
>
> Note: Remove the file when not needed in future.
